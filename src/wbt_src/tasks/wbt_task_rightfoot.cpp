...
void RightFoot_Task::getTaskJacobian(const sejong::Vector &q_state, sejong::Matrix & Jt){
	sejong::Matrix Jtmp;
    robot_model_->getFullJacobian(q_state, SJLinkID::LK_rightCOP_Frame, Jtmp);
    Jt = Jtmp; //Jtmp.block(3, 0, 3, NUM_QDOT);
}

void RightFoot_Task::getTaskJacobianDotQdot(const sejong::Vector &q_state, 
